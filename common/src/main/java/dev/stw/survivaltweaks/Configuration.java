...
public enum Configuration {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("ST Configuration");

    private final static Gson GSON = new GsonBuilder()
            .setPrettyPrinting()
            .create();

    private final static String CONFIG_FILE_NAME = "stweaks-client.json";

    private final List<ConfigValue<?>> values = new ArrayList<>();

    public final ConfigValue<Boolean> showOverlay = bool("showOverlay", () -> true)
            .comment("This allows you hide or show the overlay in the top right of the screen when scanning is enabled")
            .build();

    public final ConfigValue<Integer> radius = integer("radius", () -> 2)
            .comment("The radius area (in chunks in each direction) that the scanner will search within.")
    ...
    public final ConfigValue<Boolean> lavaActive = bool("lavaActive", () -> false)
            .comment("When true, lava will automatically be added to the scan list when scanning is enabled.")
                    .build();
    ...
    public void load() {
        JsonObject rootData;
        try {
            var configPath = SurvivalTweaks.XPLAT.configPath().get().resolve(CONFIG_FILE_NAME);
