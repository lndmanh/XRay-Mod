...
public class ChunkScanTask implements Runnable {
    ...
    public ChunkScanTask(Level level, ChunkPos pos, long scanGeneration) {
    ...
    public void run() {
        ...
        final Set<dev.stw.survivaltweaks.core.scanner.ScanStore.ActiveScanTarget> activeTargets = ScanController.INSTANCE.scanStore().activeScanTargets();
            ...
            for (int l = startZ; l < startZ + 16; l++) {
                for (int m = level.getMinY(); m < level.getMaxY(); m++) {
                    ...
                    for (var target : activeTargets) {
                        if (target.matches(state, fluidState)) {
        ...
        if (scanGeneration == ScanController.INSTANCE.scanGeneration.get()) {
