...
public class OutlineRender {
	...
	public static void renderBlocks(PoseStack poseStack) {
		if (!ScanController.INSTANCE.isActive() || Minecraft.getInstance().player == null) {
			return;
		}
		...
		for (var chunkWithBlockData : entries) {
			...
			if (holder == null) {
				...
				try (MeshData meshData = bufferBuilder.buildOrThrow()) {
					...
					GpuBuffer vertexBuffer = RenderSystem.getDevice()
							.createBuffer(() -> "ST vertex buffer", GpuBuffer.USAGE_VERTEX, meshData.vertexBuffer());
			...
			try (RenderPass renderPass = RenderSystem.getDevice()
					...
					.createRenderPass(() -> "st_overlay", colorTextureView, OptionalInt.empty(), depthTextureView, OptionalDouble.empty())) {
